...
const CategoryPieChart = ({ transactions }) => {
  // Filter out expenses and aggregate by category
...
const CategoryPieChartI = ({ transactions }) => {
  // Filter out income and aggregate by category
  ...
  const totalIncome = Object.values(incomeByCategory).reduce((acc, value) => acc + value, 0);

  // Map data to include percentage
  const chartData = Object.keys(incomeByCategory).map((category) => ({
    name: category,
    value: incomeByCategory[category],
    percentage: ((incomeByCategory[category] / totalIncome) * 100).toFixed(2),
  }));
    ...
    <div className="flex-1 flex flex-col items-center mt-8">
      ...
      {chartData.length > 0 ? (
        <PieChart width={400} height={400}>
          ...
          <Tooltip formatter={(value) => `${value} (${(value / totalIncome * 100).toFixed(2)}%)`} />
          <Legend />
        </PieChart>
      ) : (
...
const Dashboard = ({ transactions }) => {
  return (
    <div className="flex flex-wrap justify-between gap-8">
      <div className="w-full sm:w-[48%] md:w-[48%] lg:w-[48%] xl:w-[48%]">
        <CategoryPieChart transactions={transactions} />
      </div>
      <div className="w-full sm:w-[48%] md:w-[48%] lg:w-[48%] xl:w-[48%]">
        <CategoryPieChartI transactions={transactions} />
      </div>
    </div>
  );
};
